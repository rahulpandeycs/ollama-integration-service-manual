...
Create a player with `POST /v1/players`. The `playerId` is generated by the service; `firstName` and `lastName` are required.

```shell
curl --location 'http://localhost:8080/v1/players' \
